...
from stacking_models import Model1Xgb, Model1NN, Model2Linear


# 学習データに対する「目的変数を知らない」予測値と、テストデータに対する予測値を返す関数
def predict_cv(model, train_x, train_y):
    preds = []
    # preds_test = []
    va_idxes = []
...
pred_train_1a = predict_cv(model_1a, train_x, train_y)

model_1b = Model1NN()
# pred_train_1b, pred_test_1b = predict_cv(model_1b, train_x_nn, train_y, test_x_nn)
pred_train_1b = predict_cv(model_1b, train_x_nn, train_y)
...
pred_train_2 = predict_cv(model_2, train_x_2, train_y)
print(f'logloss: {log_loss(train_y, pred_train_2, eps=1e-7):.4f}')

score = f1_score(train_y, pred_train_2, average="macro")
print(f"f1_score={score}\n")
print(classification_report(train_y, pred_train_2))
print(confusion_matrix(train_y, pred_train_2))
